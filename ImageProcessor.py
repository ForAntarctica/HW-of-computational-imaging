...
class ImageProcessor:
    ...
    def PGM_denoising(self, index_image,lam,t):
        # 近段梯度下降去噪点函数
        image_array = np.array(self.noisy_images[index_image])
        # 这里正则项选择L1范数，保真项为||y-x||^2
        k = 0
        image_array_k_old = image_array
        image_array_k = image_array
        while k < 1e5:
            image_array_k_temp = image_array_k_old - t * (image_array_k_old - image_array)
            # 软阈值函数
            for i, row in enumerate(image_array_k_temp):
                for j, pixel in enumerate(row):
                    for k, channel in enumerate(pixel):
                        if channel > lam * t:
                            image_array_k[i][j][k] = channel - lam * t
                        elif abs(channel) < lam * t:
                            image_array_k[i][j][k] = 0
                        elif channel < -lam * t:
                            image_array_k[i][j][k] = channel + lam * t                  
#                    if (pixel > lam * t).all():
#                        image_array_k[i][j] = pixel - lam * t
#                    elif (pixel < -lam * t).all():
#                        image_array_k[i][j] = pixel + lam * t
#                    else:
#                        image_array_k[i][j] = 0
    ...
    def ADMM_denoising(self, index_image, lam, t1, t2, alpha_k):
        ...
        while k < 1e6:
            ...
            for i, row in enumerate(u_k_temp):
                for j, pixel in enumerate(row):
                    for k, channel in enumerate(pixel):
                        if channel > lam * t1:
                            ...
                            u_k[i][j][k] = channel + lam * t1
#                   if (pixel > lam * t1).all():
#                        u_k[i][j] = pixel - lam * t1
#                    elif (pixel < -lam * t1).all():
#                        u_k[i][j] = pixel + lam * t1
#                    else:
#                        u_k[i][j] = 0
